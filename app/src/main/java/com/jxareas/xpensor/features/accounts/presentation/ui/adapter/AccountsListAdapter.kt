...
) : ListAdapter<AccountListItem, AccountsViewHolder>(
    AsyncDifferConfig.Builder(AccountDiffCallback).build()
) {

    private var onClickListener: AccountsListAdapter.OnClickListener? = null

    class OnClickListener(val clickListener: (account: AccountListItem) -> Unit) {
        fun onClick(account: AccountListItem) = clickListener(account)
    }
